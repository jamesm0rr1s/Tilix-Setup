...
windowHeight=$(dconf read /com/gexperts/Tilix/quake-height-percent)
...
if [ $argumentCount -ne 1 ] || ! [[ "$1" =~ ^[0-9]+$ ]] || [ $1 -gt 100 ]; then
	...
	if [ "$windowHeight" = "30" ]; then
		...
		dconf write /com/gexperts/Tilix/quake-height-percent 70

	# The window height is not 30
	else

		# Set the window height to 30
		dconf write /com/gexperts/Tilix/quake-height-percent 30
	fi
...
# A window height was provided
	...
	dconf write /com/gexperts/Tilix/quake-height-percent $1
fi